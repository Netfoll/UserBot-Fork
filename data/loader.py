...
class Modules:
            ...
            mods = [
                ...
                for mod in filter(
                    ...
                    os.listdir(os.path.join(
                        utils.get_base_dir(), MODULES_NAME)),
                )
            ]
    ...
    async def _register_modules(
        ...
        for mod in modules:
            try:
                ...
                with open(mod, "r", encoding='utf8') as file:
        ...
        await self.client(
            UpdateNotifySettingsRequest(
                ...
                settings=InputPeerNotifySettings(
                    show_previews=False, silent=False),
            )
        )
        ...
        *,
        ...
        if re.search(r"# ?scope: ?hikka_min", code):
            ...
            if version.__version__ < ver:
                _raise(
                    RuntimeError(
                        f"Library requires Hikka WE version {'{}.{}.{}'.format(*ver)}+"
                    )
                )
        ...
        for old_lib in self.libraries:
            if old_lib.name == lib_obj.name and (
                ...
                logger.debug(
                    "Using existing instance of library %s", old_lib.name)
                return old_lib
        ...
        if hasattr(lib_obj, "config"):
            if not isinstance(lib_obj.config, LibraryConfig):
                _raise(
                    RuntimeError(
                        "Library config must be a `LibraryConfig` instance")
                )
    ...
    def send_config_one(self, mod: Module, skip_hook: bool = False):
        ...
        try:
            ...
            logger.exception(
                "Failed to send mod config complete signal due to %s", e)
            raise
    ...
    async def send_ready(self):
        ...
        try:
            ...
            logger.exception(
                "Failed to send mod init complete signal due to %s", e)
