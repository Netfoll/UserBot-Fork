...
class SpotifyMod(loader.Module):
    ...
    async def snowcmd(self, message: Message):
        ...
        try:
            result = (
                (
                    "<emoji document_id=5463133353232310209>💃</emoji>"
            ...
            icon = (
                ...
                else "<emoji document_id=5407080065764695950>📱</emoji>"
            )
            result += (
                f"{{is_flac}}\n\n{icon} <b>{self.strings('currently_on')}</b>"
                f" <code>iPhone 14 Pro smartphone</code>"
                if device
                else ""
            )
            result += (
            ...
            result += (
                "\n\n<emoji document_id=5282852032263233269>🎵</emoji> <b><a"
