...
class NetfollInfoMod(loader.Module):
    ...
    strings = {
        ...
        "_cfg_cst_msg": (
            ...
            " {platform}, {upd}, {uptime}, {cpu_usage}, {ram_usage}, {mods} keywords"
        ),
    ...
    def _render_info(self, inline: bool) -> str:
        ...
        _version = (
            f'<i>{version.branch} {".".join(list(map(str, list(version.netver))))}</i>'
        )
        prefix = f"«<code>{utils.escape_html(self.get_prefix())}</code>»"
        mods = len(self.allmodules.modules)
    ...
    @loader.command(
        ...
        uk_doc="Надіслати інформацію на кшталт 'Що таке Netfoll?'",
    )
    ...
    @loader.command(
        ...
        uk_doc="<текст> - Змінити текст у .info",
    )
